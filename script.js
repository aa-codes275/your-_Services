...
const logoTile = (src, name, avatar = false) =>
  `<div class="${avatar ? 'avatar-tile' : 'logo-tile'}">${src ? `<img src="${esc(src)}" alt="" loading="lazy" decoding="async" />` : esc(initials(name))}</div>`;
  ...
  <main>
    <section class="hero-section">
      <div class="hero-media" aria-hidden="true">
        <video id="hero-video" poster="hero-poster.jpg" autoplay muted loop playsinline preload="metadata" disablepictureinpicture tabindex="-1">
...
function initHeroVideo() {
  ...
  // احترام توفير البيانات والشبكات البطيئة جداً والحركة المخفّضة فقط
  const conn = navigator.connection || navigator.mozConnection || navigator.webkitConnection;
  const slow = conn && (conn.saveData || /^(slow-2g|2g)$/.test(conn.effectiveType || ''));
  const reduced = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (slow || reduced) { video.remove(); return; }
  ...
  video.addEventListener('canplay', () => { video.classList.add('is-ready'); video.play().catch(() => {}); }, { once: true });

  // تشغيل عند أول لمسة/تمرير في حال منع المتصفح التشغيل التلقائي
  const unlock = () => { video.play().catch(() => {}); };
  ['touchstart', 'pointerdown', 'scroll'].forEach((ev) =>
    window.addEventListener(ev, unlock, { once: true, passive: true })
  );

  // إيقاف الفيديو عندما يخرج من الشاشة (يوفر بطارية وأداء على الموبايل)
  if ('IntersectionObserver' in window) {
    const io = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) { start(); video.play().catch(() => {}); }
  ...
  // البدء مبكراً بعد أول رسم للصفحة
  requestAnimationFrame(() => setTimeout(start, 60));
}

/* ---------- الرجوع لأعلى الصفحة عند تغيير المسار ---------- */
